...
def preprocess_images(lt_datamodule: DataLoader) -> (np.ndarray, list):
    ...
    """
     # Initialize empty lists and an image counter
    all_images = []
    all_labels = []

    # Loop over each batch of images and labels by enumerating over the lt_datamodule
    for batch, (img, label) in enumerate(lt_datamodule):
        # Set batch size to the number of images in the batch
        bs = len(img)
        ...
        for i in range(bs):
            ...
            # To convert one hot encoded labels to a class index
            # Use np.argmax() function
            class_index = np.argmax(label[i], axis=0)
            all_labels.append(class_index)

            # Append the flattened image to the list of all flattened images
            all_images.append(flattened_img)
...
def perform_pca(X_flat: np.ndarray, n_components: int) -> tuple:
    ...
    # Initialize a PCA object and specify the number of components to keep
    pca = PCA(n_components=n_components)

    # Fit the PCA object to the flattened images
    pca.fit(X_flat)

    # Apply dimensionality reduction to the flattened images
    X_pca = pca.transform(X_flat)

    # Return the PCA object and the compressed image data
    return pca, X_pca
...
def perform_tsne(X_reduced_dim: np.ndarray,
    ...
    # Fit the TSNE object to X_reduced_dim
    X_tsne = tsne.fit_transform(X_reduced_dim)
...
def create_tsne_cp_df(X_tsne: np.ndarray,
    ...
    df_cols.append('particle_type')
    cps_df['particle_type'] = cps_df['target'].map(particle_types)
